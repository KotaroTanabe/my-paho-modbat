...
class NewHouse extends Model {
  ...
  class Controller extends Model {
    ...
    var updaterModel: Updater = _

    "init" -> "run" := {
      instance.foreach(_.connect())
      //updater.start()
      updaterModel = new Updater()
      launch(updaterModel)
      ready = true
    } label "connect"
    "run" -> "run" := {
    ...
    */
    ...
    class Updater extends Model {
      ...
      object updater extends MbtThread {
        override def mbtRun() {
          while(!end) {
            instance.foreach(_.control())
            mbtSleep(scale(interval))
          }
        }
      }
    }
      ...
      object updater extends MbtThread {
        override def mbtRun() {
          while(!end) {
            ...
            mbtSleep(scale(Environment.interval))
          }
        }
      }
    }
  ...
  class Thermometer(val i: Int, val j: Int, val port: Int = 1883) extends Model {
    ...
    var publisherModel: PublisherModel = _

    "init" -> "run" := {
      instance.foreach(_.connect())
      ready = true
      publisherModel = new PublisherModel()
      launch(publisherModel)
    } label "connect"
    ...
    class PublisherModel extends Model {
      ...
      object publisher extends MbtThread {
        override def mbtRun() {
          while(!end) {
            //instance.foreach(_.publish())
            instance.foreach(_.publish(this))
            mbtSleep(scale(interval))
          }
        }
      }
