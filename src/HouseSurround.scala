...
class HouseSurround extends Model {
  ...
  class Environment(val prefix: String) {
    ...
    val writer = new OutputStreamWriter(new FileOutputStream(s"../log/HouseSurround/${prefix}.dat"))
    var maxDiff = 0.0
  ...
  class Controller extends Model {
    ...
    class Instance(val env: Environment) {
      ...
      val measuredTemperatures = Array.fill[Double](height, width){Environment.temperatureOut}
      ...
      object listner extends MqttCallback {
        ...
        def messageArrived(topic: String, msg: MqttMessage) {
          ...
          receiveTemperature(i, j, msg.toString.toDouble)
        }
      }
      def receiveTemperature(i: Int, j: Int, value: Double) {
        measuredTemperatures(i)(j) = value
      }
      def connect() {
    ...
    class InstanceSurround(env: Environment) extends Instance(env) {
      val threshold = 2.0
      override def control() {
        var dtokens: List[IMqttDeliveryToken] = Nil
        for(i <- 0 until height) {
          for(j <- 0 until width) {
            val t0 = measuredTemperatures(i)(j)
            var count = 0
            var sum = 0.0
            for((di, dj) <- Environment.dirs) {
              val si = i + di
              ...
              if(0 <= si && si < height && 0 <= sj && sj < width) {
                count += 1
                sum += measuredTemperatures(si)(sj)
              }
            }
            val mean = sum / count
            val t = if((t0 - mean).abs <= threshold) t0 else mean
            if(t < target - sensitivity) {
              dtokens ::= setAc(i, j, 1)
            } else if(t > target + sensitivity) {
              dtokens ::= setAc(i, j, -1)
  ...
  class Thermometer(val i: Int, val j: Int, val port: Int = 1883) extends Model {
    ...
    var broken = false
    val instance = Array(
      new FragileInstance(i, j, envs(0)),
  ...
  class AirConditioner(val i: Int, val j: Int, val port: Int = 1883) extends Model {
    ...
    class FragileInstance(i: Int, j: Int, env: Environment)
      ...
      override def setMode(newMode: Int) {
        storedMode = newMode
        env.acMode(i)(j) = if(broken) choose(-1, 2) else newMode
      }
    }
  }
