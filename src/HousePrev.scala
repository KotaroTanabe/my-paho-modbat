...
class HousePrev extends Model {
  ...
  class Environment(val prefix: String) {
    ...
    val writer = new OutputStreamWriter(new FileOutputStream(s"../log/HousePrev/${prefix}.dat"))
    var maxDiff = 0.0
  ...
  class Controller extends Model {
    ...
    class InstancePrev(env: Environment) extends Instance(env) {
      val threshold = 2.0
      override def receiveTemperature(i: Int, j: Int, value: Double) {
        val prev = measuredTemperatures(i)(j)
        if((value - prev).abs <= threshold) {
          measuredTemperatures(i)(j) = value
        }
      }
    }
