...
class MBT (val model: Model, val trans: List[Transition]) {
  ...
  @volatile private var stayVal = false
  def staying = {
    stayVal
  }
  def setStay(b: Boolean) = {
    if(stayVal == b) Log.fine(name + "staying already set to " + b)
    stayVal = b
    if(Modbat.debug) Log.fine("set " + name + " staying = " + b)
  }
  //@volatile var executing = false
  @volatile private var sleepVal = false
  def sleeping = {
    sleepVal
  }
  def setSleep(b: Boolean) = {
    if(sleepVal == b) Log.fine(name + "sleeping already set to "+ b)
    sleepVal = b
    if(Modbat.debug) Log.fine("set " + name + " sleeping = "+ b)
  }
  @volatile var runningThread: Option[MbtThread] = None
    ...
    runningThread match {
      ...
      case Some(t) => {
        try {
          ...
          case e: Exception => {
            e.printStackTrace()
            //Log.error("error in Thread in model" + this.classname)
            runningThread = None
            false
          }
        }
