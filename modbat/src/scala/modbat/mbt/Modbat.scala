...
object Modbat {
  ...
  val timeout = 1000
  val debug = true
  //val mainLock: Lock = new ReentrantLock()
...
/* TODO: add another field that allows invokeTransition to override the
   choice to be taken. Perhaps a queue of transitions in case of multiple
   threads invoking transitions in a callback. Those invocations should take
   predence over the default successors, except if givenModel != null. */
  def allSuccessors(givenModel: MBT): Array[(MBT, Transition)] = {
    ...
    if (givenModel == null) {
      do {
        if(callAgain) Log.fine("passed callAgain")
        callAgain = false
        //mainLock.lock()
//        MBT.stayLock.synchronized {
          // TODO: allow selection to be overridden by invokeTransition
          var (executing, notExecuting) = MBT.launchedModels partition (_.executing)
          var (sleeping, notSleeping) = executing partition (_.sleeping)
          var (staying, notStaying) = notExecuting partition (_.staying)

          if(debug) {
            var str: String = "Staying: "
            for(m <- staying) {
              str += m.name + ", "
            }
            Log.fine(str)
          ...
          if(result.isEmpty) {
            if(!notSleeping.isEmpty) {
              if(debug) Log.fine("wait for executing thread")
              Timekeeper.earliest match {
                case Some(waitTime) => 
                  if(waitTime > 0) {
                ...
                case None => {
                  if(debug) {
                    for(m <- notSleeping) {
                      Log.fine("notSleeping: " + m.className)
                    }
                  }
                  waitLock.synchronized {
                    //mainLock.unlock()
              ...
              callAgain = true
            } else if (!staying.isEmpty || !sleeping.isEmpty) {
              Timekeeper.skip
              callAgain = true
            }
          }
//        }//stayLock.synchronized.end
        //mainLock.unlock()
      } while (callAgain)
      //if(callAgain) return allSuccessors(givenModel)//stayLockを一旦解放する
    } else {
  ...
  def exploreSuccessors: (TransitionResult, RecordedTransition) = {
    ...
    while (!successors.isEmpty && totalW > 0) {
      ...
      while (!MBT.transitionQueue.isEmpty && successor == null) {
        val (model, label) = MBT.transitionQueue.dequeue //取り出した遷移は、実行可能でなかったらQueueから消える
    ...
    //(!successors.isEmpty && totalW > 0)ここまで
    //if(debug) Log.fine("no more transition, this simulation ends.")
